...
let rec copyConstPropFoldExp (vtable : VarTable)
    ...
    match e with
        ...
        | Var (name, pos) ->
            let look_up = SymTab.lookup name vtable
            match look_up with
            | None -> Var(name, pos)
            | Some v ->
                match v with
                | VarProp vp -> Var(vp, pos)
                | ConstProp cp -> Constant(cp, pos)
            (* TODO project task 3:
        ...
        | Index (name, e, t, pos) ->
          ...
          match (e', look_up) with
          | (Constant (IntVal i, _), Some (ConstProp (ArrayVal(lst, _)))) -> Constant (lst.[i], pos)
          | (Var(name', _), Some(ConstProp (ArrayVal(lst, _)))) -> Var(name', pos)
          | otherwise -> Index (name, e', t, pos)
            (* TODO project task 3:
        ...
        | Let (Dec (name, e, decpos), body, pos) ->
            ...
            match e' with
                ...
                | Constant (const_name, const_pos) ->
                    let vtable' = SymTab.bind name (ConstProp const_name)  vtable
                    let body' = copyConstPropFoldExp vtable' body
                    Let(Dec(name, e', decpos), body', pos)
                    ...
                    (* TODO project task 3:
                        Hint: I have discovered a constant-copy statement `let x = 5`.
                              ...
                              and optimize the `body` of the let.
                    *)
                | Let (Dec (let_name, e1, let_decpos), e2, let_pos) ->


                    let let_body' = Let(Dec(name, e2, let_decpos), body, let_pos)
                    let _exp = Let(Dec(let_name, e1, decpos), let_body', pos)
                    copyConstPropFoldExp vtable _exp
