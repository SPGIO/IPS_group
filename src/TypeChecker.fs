...
and checkExp  (ftab : FunTable)
    ...
    *)
    ...
    | Not (e1, pos) ->
        let (t1, e1_dec) = checkExp ftab vtab e1
        match t1 with
          | Bool -> (Bool, Not(e1_dec, pos))
          | Int -> (Bool, Not(e1_dec, pos))
          | Char -> (Bool, Not(e1_dec, pos))
          | _ -> raise(MyError("Invalid type", pos))
    ...
    | Negate (e1, pos) ->
      ...
      if Int = t1 then
        ...
        raise(MyError ("Can only negate numbers", pos))
    ...
    | Apply ("length", [arg], pos) ->
        ...
        match targ with
          ...
          | _ -> raise (MyError ("Function length call on non-array type", pos))
    ...
    *)
    | Scan (f, n_exp, arr_exp, _, pos) ->
        ...
        if   arr_type = f_arg_type && elem_type = n_type then
             (arr_type, Scan (f', n_dec, arr_dec, elem_type, pos))
        elif arr_type = f_arg_type then
             raise (err ("neutral element", n_type))
        else raise (err ("array element", elem_type))
